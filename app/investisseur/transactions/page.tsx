...
export default function TransactionsDashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      <Head>
        <title>Transactions - AgriTech Sénégal</title>
        <meta name="description" content="Gérez vos transactions sur AgriTech Sénégal" />
      ...
      <div className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-50 w-64 bg-green-800 text-white transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:block`}>
        ...
        <nav className="p-4">
          <ul className="space-y-2">
            ...
            <li>
              <Link href="/investisseur/transactions" className="flex items-center p-2 rounded-md bg-green-700 hover:bg-green-600">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <main className="p-6">
          ...
          <section className="mb-8">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">Aperçu financier</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white p-4 rounded-lg shadow">
                <h4 className="text-sm font-medium text-gray-500">Total investi</h4>
                <p className="text-2xl font-bold text-gray-800">{formatCurrency(transactionSummary.totalInvested)}</p>
                <p className="text-sm text-gray-500">{transactionSummary.activeInvestments} investissements actifs</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg shadow">
                <h4 className="text-sm font-medium text-gray-500">Total retiré</h4>
                <p className="text-2xl font-bold text-red-600">{formatCurrency(transactionSummary.totalWithdrawn)}</p>
                <p className="text-sm text-gray-500">Sur l&apos;année 2025</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg shadow">
                <h4 className="text-sm font-medium text-gray-500">Rendements en attente</h4>
                <p className="text-2xl font-bold text-blue-600">{formatCurrency(transactionSummary.pendingReturns)}</p>
                <p className="text-sm text-gray-500">À percevoir prochainement</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg shadow">
                <h4 className="text-sm font-medium text-gray-500">Solde disponible</h4>
                <p className="text-2xl font-bold text-green-600">{formatCurrency(1200000)}</p>
                <p className="text-sm text-gray-500">Pour de nouveaux investissements</p>
              </div>
            </div>
          </section>
              ...
              <div className="flex flex-wrap justify-between items-center">
                <div className="flex space-x-2 mb-4 md:mb-0">
                  <button 
                    ...
                    className={`px-4 py-2 rounded-md ${viewMode === 'all' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Toutes
                  </button>
                  <button 
                    onClick={() => setViewMode('investments')} 
                    className={`px-4 py-2 rounded-md ${viewMode === 'investments' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Investissements
                  </button>
                  <button 
                    onClick={() => setViewMode('withdrawals')} 
                    className={`px-4 py-2 rounded-md ${viewMode === 'withdrawals' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Retraits
                  </button>
                  <button 
                    onClick={() => setViewMode('returns')} 
                    className={`px-4 py-2 rounded-md ${viewMode === 'returns' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    Rendements
                  </button>
                </div>
                ...
                <div className="flex space-x-4">
                  <div className="relative">
                    <button className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">
                      ...
                      <span>{timeFilter === 'all' ? 'Toute période' : timeFilter === 'month' ? 'Ce mois' : timeFilter === 'quarter' ? 'Ce trimestre' : 'Cette année'}</span>
                          ...
                          <td className="px-6 py-4">
                            {transaction.type === 'investment' ? (
                              <div className="flex items-center">
                                ...
                                <div>
                                  <div className="text-sm font-medium text-gray-900">{transaction.projectTitle}</div>
                                  {transaction.farmer && (
                                    <div className="text-sm text-gray-500">Agriculteur: {transaction.farmer}</div>
                                  )}
                                </div>
                              </div>
                            ) : transaction.type === 'return' ? (
                              <div>
                                <div className="text-sm font-medium text-gray-900">Rendement</div>
                                {transaction.projectTitle && (
                                  <div className="text-sm text-gray-500">Projet: {transaction.projectTitle}</div>
                                )}
                              </div>
                            ) : (
                              <div className="text-sm font-medium text-gray-900">
                                {transaction.type === 'withdrawal' ? 'Retrait vers compte bancaire' : 'Transaction'}
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              ...
                              <span className={`px-2 py-1 text-xs rounded-full ${typeInfo.badgeClass}`}>
                                {typeInfo.label}
                              </span>
                            </div>
                          </td>
              ...
              <div className="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-700">
                    Affichage de <span className="font-medium">1</span> à <span className="font-medium">{filteredTransactions.length}</span> sur <span className="font-medium">{transactionHistory.length}</span> résultats
                  </div>
